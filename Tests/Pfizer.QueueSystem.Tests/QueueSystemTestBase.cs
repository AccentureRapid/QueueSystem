...
namespace Pfizer.QueueSystem.Tests
{
    public abstract class QueueSystemTestBase : AbpIntegratedTestBase<QueueSystemTestModule>
    {
        ...
        protected void UsingDbContext(Action<QueueSystemDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<QueueSystemDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<QueueSystemDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<QueueSystemDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<QueueSystemDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<QueueSystemDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<QueueSystemDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<QueueSystemDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<QueueSystemDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<QueueSystemDbContext, Task<T>> func)
        {
            T result;
